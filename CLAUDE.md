...
- **Queue driver**: `database` — los clicks se registran de forma asíncrona con `TrackClickJob`
...
2. `RedirectController` busca el QR activo, despacha `TrackClickJob` y devuelve `302` al destino
3. El job (en cola `database`) resuelve IP → país/ciudad, User-Agent → device/OS/browser, guarda en `clicks`
...
# Worker de colas (terminal separada)
